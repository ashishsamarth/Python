...
class CustomOpenpyxl:
    ...
    def active_ws(self, _ws_name: str):
        '''
    ...
    def ref_col_idx_name_map(self, _header_row_num: int) -> dict:
        '''
    ...
    def ref_col_name_idx_map(self, _header_row_num: int) -> dict:
        '''
    ...
    def add_multiple_cols_at_the_end_to_active_ws(self, _header_row_num: int, *_col_names):
        '''
    ...
    def auto_format_active_ws(self, _ws_name: str, _header_row_num: int):
        '''
    ...
    def change_cell_color_by_cell_value_near_match(self, _header_row_num: int, _col_name: str, _cell_value, _fill_color: str):
        '''
        ...
        _col_idx = column_index_from_string(self.ref_col_name_letter_map(_header_row_num)[_col_name])
        # Get the list of indexes where near match of searched value if found excluding the header row
        _row_idx = self.get_row_idx_lst_based_on_search_val_specific_col_near_match(_header_row_num, _col_name, _cell_value)
        # Set the color fill based on user selected color
        _my_color = PatternFill(start_color=self._my_color_map[_fill_color], end_color=self._my_color_map[_fill_color], fill_type='solid')
        # Iterate over the list of row indexes identified earlier
        for _row_num in _row_idx:
    ...
    def change_row_color_by_cell_value_exact_match(self, _header_row_num: int, _col_name: str, _cell_value, _fill_color: str):
        '''
        Method to change row color based on an exact match of cell value in particular column
        Arguments to this method: - Row number for header row, Column name, value of cell, user selected fill color
        '''
    ...
    def change_row_color_by_cell_value_near_match(self, _header_row_num: int, _col_name: str, _cell_value, _fill_color: str):
        '''
        Method to change row color based on a near match of cell value in particular column
        Arguments to this method: - Row number for header row, Column name, value of cell, user selected fill color
        '''
    ...
    def change_row_color_empty_cell_in_col(self, _header_row_num: int, _col_name: str, _fill_color: str):
        '''
    ...
    def del_row_by_val_in_col_near_match(self, _header_row_num: int, _col_name: str, _search_val):
        ...
        if _col_name in self.ref_col_name_idx_map(_header_row_num).keys():
            ...
            if self.get_row_idx_lst_based_on_search_val_specific_col_near_match(_header_row_num, _col_name, _search_val):
                # Get the list of indexes based on match condition
                _depleting_idx = [_ for _ in self.get_row_idx_lst_based_on_search_val_specific_col_near_match(_header_row_num, _col_name, _search_val)]
                # Iterate for all values in _depleting_idx
                while _depleting_idx:
                    # Delete the row for the iterated index value
                    # Start from the last index value since it will not change position of index, post delete
    ...
    def drop_col_by_letter_in_active_ws(self, _ws_name: str, _col_letter: str):
        '''
    ...
    def filter_rows_by_val_in_any_col_in_active_ws(self, _header_row_num: int, _filter_val):
        '''
    ...
    def filter_rows_by_val_in_a_col_in_active_ws(self, _header_row_num: int, _filter_col_name: str, _filter_val):
        '''
    ...
    def filter_rows_by_val_not_in_a_col_in_active_ws(self, _header_row_num: int, _filter_col_name: str, _filter_val):
        '''
        Method to filter rows based on a value present in a specific column of worksheet
        Arguments to this method are: - value used as filter, column idx where filter will be applied
        '''
    ...
    def format_header_row(self, _header_row_num: int, _cell_color: str, _font_color: str):
        '''
    ...
    def get_specific_col_val_by_col_name_in_active_ws(self, _header_row_num: int, _col_name: str):
        '''
    ...
    def get_specific_col_val_as_list_by_col_idx_in_active_ws(self, _header_row_num: int, _col_idx: int) -> list:
        '''
    ...
    def get_specific_col_val_as_str_by_col_idx_in_active_ws(self, _header_row_num: int, _col_idx: int) -> str:
        '''
        Method to get column values for a given column index
        Arguments to this method are:- Row number for header row & column index
        Note:- Column index starts with 1
        ...
        assert _col_idx >= 1, 'ValueError: Row or column values must be at least 1'
        try:
            return [str(_col_value[0]) for _col_value in self.my_base_active_ws.iter_rows(min_row=_header_row_num + 1,
                                                                                     min_col=_col_idx,
                                                                                     max_col=_col_idx,
                                                                                     values_only=True)]
        except ValueError:
    ...
    def get_specific_col_val_as_str_by_col_letter_in_active_ws(self, _header_row_num, _col_letter) -> str:
        '''
        Method to get column values for a given column letter
        Arguments to this method are:- Row number for header row & column letter
        '''
    ...
    def get_row_idx_lst_of_empty_rows_specific_col(self, _header_row_num: int, _col_name: str):
        '''
    ...
    def read_values_from_cell_range_active_ws(self, _header_row_num: int, _start_range: int, _end_range: int):
        '''
    ...
    def sort_asc_row_val_in_col_sep_by_delim_active_ws(self, _header_row_num: int, _col_name: str, _delim: str, _join_by_delim: str):
        '''
